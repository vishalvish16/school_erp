# Parent Portal — Full Module Plan
...
| POST | `/api/parent/change-password` | `{ current_password, new_password }` | N/A — parents use OTP, no password. Skip or use "update phone" flow. |
...
flutter analyze
flutter test
# Manual: Login as parent, navigate all screens
```
...
## 11. File Ownership (Agent Team)

| Agent | Owns | Does NOT touch |
|-------|------|----------------|
| Database | `schema.prisma`, migrations, Parent/StudentParent | Flutter, backend controllers |
| Backend | `backend/src/modules/parent/*`, auth parent flow | Prisma schema (read only), Flutter |
| Frontend | `lib/features/parent/*`, router parent routes, parent_login_screen | Backend, schema |
...
```
Database → Backend: Parent, StudentParent models, Prisma client
Backend → Frontend: API contract (URLs, response shapes, status codes)
```

Both backend and frontend MUST use the exact API contract above. No deviations without lead approval.
